...
function AtlasAcademyView() {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* HERO BANNER - Exact ATLAS Texture */}
      <div className="glass-panel" style={{ padding: '28px 32px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', borderRadius: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '20px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '4px 12px', background: 'rgba(255, 255, 255, 0.06)', border: '1px solid rgba(255, 255, 255, 0.12)', borderRadius: '20px', color: '#fff', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '10px' }}>
            <span>🎓 ATLAS ACADEMY</span>
            <span>•</span>
            <span style={{ color: 'rgba(255,255,255,0.6)' }}>Platform Mastery & Certifications</span>
          </div>
          <h1 style={{ fontSize: '26px', fontWeight: 700, color: '#fff', margin: '0 0 6px 0', letterSpacing: '-0.01em' }}>
            Atlas Academy
          </h1>
          <p style={{ color: 'rgba(255, 255, 255, 0.5)', fontSize: '13px', margin: 0, maxWidth: '640px', lineHeight: 1.5 }}>
            Master Full-Stack AI Systems, Control Center Security, Digital Marketplace Operations, and Candidate Telemetry through official ATLAS interactive tracks and verified credentials.
          </p>
        </div>

        <div style={{ display: 'flex', gap: '12px' }}>
          <div style={{ padding: '12px 20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff' }}>12</div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>Active Paths</div>
          </div>
          <div style={{ padding: '12px 20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff' }}>1,480</div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>Certificates Issued</div>
          </div>
          <div style={{ padding: '12px 20px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px', textAlign: 'center' }}>
            <div style={{ fontSize: '20px', fontWeight: 700, color: '#fff' }}>98.4%</div>
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', fontWeight: 500 }}>Skill Mastery</div>
          </div>
        </div>
      </div>

      {/* ACADEMY TOOLBAR TABS - Exact ATLAS Pill Texture */}
      <div style={{ display: 'flex', gap: '6px', background: 'rgba(255, 255, 255, 0.02)', padding: '6px', borderRadius: '30px', border: '1px solid rgba(255, 255, 255, 0.08)', width: 'fit-content', flexWrap: 'wrap' }}>
        <button
          onClick={() => setAcademyTab('paths')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'paths' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'paths' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'paths' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          🎓 Learning Paths ({learningPaths.length})
        </button>
        <button
          onClick={() => setAcademyTab('courses')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'courses' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'courses' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'courses' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          📚 Interactive Courses ({courses.length})
        </button>
        <button
          onClick={() => setAcademyTab('certificates')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'certificates' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'certificates' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'certificates' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          📜 Certificate Vault ({certificates.length})
        </button>
        <button
          onClick={() => setAcademyTab('sandbox')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'sandbox' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'sandbox' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'sandbox' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          🧪 Live AI Sandbox
        </button>
        <button
          onClick={() => setAcademyTab('playbooks')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'playbooks' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'playbooks' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'playbooks' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          📖 ATLAS Playbooks
        </button>
        <button
          onClick={() => setAcademyTab('leaderboard')}
          style={{ padding: '8px 18px', borderRadius: '24px', fontSize: '13px', fontWeight: 600, border: academyTab === 'leaderboard' ? '1px solid rgba(255,255,255,0.2)' : 'none', cursor: 'pointer', transition: 'all 0.2s', background: academyTab === 'leaderboard' ? 'rgba(255, 255, 255, 0.16)' : 'transparent', color: academyTab === 'leaderboard' ? '#fff' : 'rgba(255, 255, 255, 0.5)' }}
        >
          🏆 Leaderboard
        </button>
      </div>
              ...
              >
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                    <div style={{ fontSize: '28px', width: '44px', height: '44px', borderRadius: '12px', background: 'rgba(255, 255, 255, 0.04)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
                      {path.icon}
                    </div>
                    <span style={{ fontSize: '10px', fontWeight: 700, padding: '4px 10px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.06)', color: 'rgba(255, 255, 255, 0.8)', border: '1px solid rgba(255, 255, 255, 0.12)' }}>
                      {path.badge}
                    </span>
                  </div>

                  <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#fff', margin: '0 0 8px 0', lineHeight: 1.3 }}>
                    {path.title}
                  </h3>

                  <p style={{ fontSize: '13px', color: 'rgba(255, 255, 255, 0.5)', lineHeight: 1.5, margin: '0 0 16px 0' }}>
                    {path.desc}
                  </p>

                  <div style={{ display: 'flex', gap: '12px', fontSize: '12px', color: 'rgba(255, 255, 255, 0.4)', marginBottom: '16px' }}>
                  ...
                  {isEnrolled && (
                    <div style={{ marginBottom: '16px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: 'rgba(255, 255, 255, 0.5)', marginBottom: '4px' }}>
                        ...
                        <span style={{ color: '#fff', fontWeight: 700 }}>{path.progress}%</span>
                      </div>
                      <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px', overflow: 'hidden' }}>
                        <div style={{ width: `${path.progress}%`, height: '100%', background: '#fff', borderRadius: '3px' }}></div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  <button
                    ...
                    style={{ flex: 1, padding: '9px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', color: '#fff', fontSize: '12px', fontWeight: 600, cursor: 'pointer' }}
                  >
                    Syllabus & Info
                  </button>
                  ...
                  <button
                    ...
                    style={{ flex: 1.2, padding: '9px', background: isEnrolled ? 'rgba(255,255,255,0.18)' : 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}
                  >
      ...
      {academyTab === 'courses' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
          {courses.map((c) => (
            <div key={c.id} className="glass-panel" style={{ padding: '20px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                <div style={{ fontSize: '28px' }}>{c.icon}</div>
                <span style={{ fontSize: '11px', color: '#fff', background: 'rgba(255, 255, 255, 0.06)', padding: '3px 10px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.1)', fontWeight: 600 }}>⭐ {c.rating} ({c.students})</span>
              </div>
              <h3 style={{ fontSize: '16px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>{c.title}</h3>
              <p style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.5)', lineHeight: '1.5', marginBottom: '14px' }}>{c.desc}</p>
              <div style={{ fontSize: '11px', color: 'rgba(255, 255, 255, 0.4)', marginBottom: '14px' }}>Instructor: <strong style={{ color: '#fff' }}>{c.instructor}</strong> • {c.duration}</div>
              <button onClick={() => setSelectedCourse(c)} style={{ width: '100%', padding: '9px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '20px' }}>
            {certificates.map((cert) => (
              <div key={cert.id} className="glass-panel" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)', position: 'relative' }}>
                <div style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.6)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>VERIFIED CREDENTIAL</div>
                <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>{cert.title}</h3>
                <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '14px' }}>Issued on {cert.date} • Score: <strong style={{ color: '#fff' }}>{cert.grade}</strong></div>
                
                <div style={{ background: 'rgba(0,0,0,0.3)', padding: '10px 14px', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.08)', fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginBottom: '16px', display: 'flex', justifyContent: 'space-between' }}>
                  <span>Verification ID:</span>
                  <span style={{ color: '#fff', fontWeight: 700, fontFamily: 'monospace' }}>{cert.id}</span>
                </div>

                <button onClick={() => setViewCertificate(cert)} style={{ width: '100%', padding: '10px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer' }}>
      ...
      {/* TAB: LIVE AI SANDBOX */}
      {academyTab === 'sandbox' && (
        <div className="glass-panel" style={{ padding: '28px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
            <div>
              <span style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.05em', textTransform: 'uppercase' }}>INTERACTIVE AI LABORATORY</span>
              <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#fff', margin: '4px 0 0 0' }}>ATLAS Autonomous AI Prompt Sandbox</h2>
            </div>
            <span style={{ fontSize: '11px', background: 'rgba(255, 255, 255, 0.06)', color: '#fff', padding: '4px 12px', borderRadius: '20px', border: '1px solid rgba(255,255,255,0.12)', fontWeight: 600 }}>LIVE INFERENCE ENGINE</span>
          </div>
          ...
          <button
            ...
            style={{ padding: '10px 24px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '13px', cursor: 'pointer', opacity: sandboxLoading ? 0.7 : 1 }}
          >
            {sandboxLoading ? '⏳ Running Neural Evaluation...' : '⚡ Execute AI Inference Test'}
          </button>

          {sandboxOutput && (
            <div style={{ marginTop: '20px', padding: '18px', background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '14px', fontSize: '12px', color: '#fff', fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
              {sandboxOutput}
            </div>
          )}
        </div>
      ...
      {academyTab === 'playbooks' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '20px' }}>
          <div className="glass-panel" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
            <div style={{ fontSize: '32px', marginBottom: '12px' }}>📘</div>
            <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>The ATLAS Software Engineering Bible</h3>
            <p style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.5)', lineHeight: '1.5', marginBottom: '16px' }}>Comprehensive architecture manual covering FastAPI async routes, PostgreSQL schemas, and Docker Compose deployment.</p>
            <button onClick={() => alert('Downloading ATLAS Architecture Blueprint (PDF)...')} style={{ padding: '9px 18px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>📥 Download Blueprint (PDF)</button>
          </div>

          <div className="glass-panel" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
            <div style={{ fontSize: '32px', marginBottom: '12px' }}>🧠</div>
            <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>Prompt Engineering for AI Candidate Matching</h3>
            <p style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.5)', lineHeight: '1.5', marginBottom: '16px' }}>Proven prompt templates for LLM resume parsing, candidate scoring, and automated interview question generation.</p>
            <button onClick={() => alert('Downloading Prompt Engineering Playbook (PDF)...')} style={{ padding: '9px 18px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>📥 Download Playbook (PDF)</button>
          </div>

          <div className="glass-panel" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
            <div style={{ fontSize: '32px', marginBottom: '12px' }}>🔒</div>
            <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#fff', marginBottom: '6px' }}>ATLAS Security & Creator Passcode Hardening</h3>
            <p style={{ fontSize: '12px', color: 'rgba(255, 255, 255, 0.5)', lineHeight: '1.5', marginBottom: '16px' }}>Best practices for configuring Creator Key authentication, admin terminal access, and database encryption.</p>
            <button onClick={() => alert('Downloading Security Hardening Guide (PDF)...')} style={{ padding: '9px 18px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '12px', cursor: 'pointer' }}>📥 Download Security Guide (PDF)</button>
          </div>
        </div>
      )}
        ...
        <div className="glass-panel" style={{ padding: '24px', borderRadius: '20px', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.08)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {leaderboard.map((user) => (
              <div key={user.rank} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 18px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: '14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                  <div style={{ fontSize: '15px', fontWeight: 700, width: '28px', color: 'rgba(255,255,255,0.5)' }}>#{user.rank}</div>
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 700, color: '#fff' }}>{user.name}</div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>{user.title} • {user.courses} Courses Completed</div>
                  </div>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                  <span style={{ fontSize: '11px', fontWeight: 600, padding: '3px 10px', borderRadius: '20px', background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.8)' }}>{user.badge}</span>
                  <span style={{ fontSize: '15px', fontWeight: 700, color: '#fff' }}>{user.xp}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedCourse && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(8px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '520px', padding: '28px', borderRadius: '20px', background: '#090a0f', border: '1px solid rgba(255,255,255,0.12)', position: 'relative' }}>
            <button onClick={() => setSelectedCourse(null)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: '20px', cursor: 'pointer' }}>✕</button>

            <div style={{ fontSize: '40px', marginBottom: '12px' }}>{selectedCourse.icon || '🎓'}</div>
            <h2 style={{ fontSize: '20px', fontWeight: 700, color: '#fff', margin: '0 0 6px 0' }}>{selectedCourse.title}</h2>
            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', fontWeight: 500, marginBottom: '14px' }}>Level: {selectedCourse.level || 'Professional'} • Duration: {selectedCourse.duration}</div>
            ...
            <div style={{ background: 'rgba(255,255,255,0.03)', padding: '14px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.08)', marginBottom: '20px' }}>
              ...
              <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', display: 'flex', flexDirection: 'column', gap: '6px' }}>
            ...
            <button
              ...
              style={{ width: '100%', padding: '12px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}
            >
      ...
      {viewCertificate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div className="glass-panel" style={{ width: '100%', maxWidth: '600px', padding: '36px', borderRadius: '24px', background: '#090a0f', border: '1px solid rgba(255,255,255,0.15)', textAlign: 'center', position: 'relative' }}>
            <button onClick={() => setViewCertificate(null)} style={{ position: 'absolute', top: '16px', right: '16px', background: 'none', border: 'none', color: 'rgba(255,255,255,0.5)', fontSize: '20px', cursor: 'pointer' }}>✕</button>

            <div style={{ fontSize: '11px', fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '8px' }}>OFFICIAL CERTIFICATE OF MASTERY</div>
            <div style={{ fontSize: '26px', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', marginBottom: '20px' }}>ATLAS ACADEMY</div>

            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>This is to certify that</div>
            <div style={{ fontSize: '22px', fontWeight: 700, color: '#fff', marginBottom: '16px', textDecoration: 'underline' }}>Gaurav Patkotwar</div>

            <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.5)', marginBottom: '8px' }}>has successfully completed all coursework, labs, and capstone exams for</div>
            <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#fff', marginBottom: '20px' }}>{viewCertificate.title}</h3>

            <div style={{ display: 'flex', justifyContent: 'space-between', background: 'rgba(255,255,255,0.04)', padding: '12px 20px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.1)', fontSize: '12px', color: 'rgba(255,255,255,0.5)', marginBottom: '24px' }}>
              <div>Date: <strong style={{ color: '#fff' }}>{viewCertificate.date}</strong></div>
              <div>Grade: <strong style={{ color: '#fff' }}>{viewCertificate.grade}</strong></div>
              <div>ID: <strong style={{ color: '#fff', fontFamily: 'monospace' }}>{viewCertificate.id}</strong></div>
            </div>

            <button onClick={() => alert(`Certificate ${viewCertificate.id} saved as PDF!`)} style={{ padding: '12px 28px', background: 'rgba(255,255,255,0.12)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: '20px', color: '#fff', fontWeight: 700, fontSize: '14px', cursor: 'pointer' }}>
...
export default function App() {
 ...
 const channelColor = TV_CHANNELS.find(c => c.id === tvChannel)?.color || '#e11d48';
  ...
  <span style={{ fontSize: '20px', fontWeight: 800, color: '#fff', letterSpacing: '-0.3px' }}>Atlas Control Center</span>
  ...
  <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', margin: '0 0 20px 0' }}>Authenticate with Admin Passphrase or Creator Key to access Atlas Control Center.</p>
  <input
  type="password"
