...
const TitanLogo = ({ size = 24, style = {}, className = '' }: { size?: number; style?: React.CSSProperties; className?: string }) => (
  ...
  >
    ...
    <circle cx="70" cy="18" r="4.5" fill="#808080" />
  </svg>
);
        ...
        try {
          ...
          for (const sig of pollRes.signals) {
            ...
            if (sig.type === 'offer') {
              ...
              pcs[sender] = pc;
              // no setState — avoids re-render storm on every ICE candidate
            } 
          ...
          // no setMeetPeerConnections — ref is already updated above
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      if (isRegister) {
        ...
        if (!email || !orgName) {
          throw new Error('Please fill in corporate email and organization name');
        }
        await api.sso.loginMock(email, orgName);
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={gridStyle}>
            ...
            <div className="glass-panel" style={{ position: 'relative', overflow: 'hidden', background: '#000', borderRadius: '12px', minHeight: '200px' }}>
              <video
                ref={el => {
                  if (el && meetLocalVideoRef.current !== el) {
                    meetLocalVideoRef.current = el;
                    if (meetLocalStream) el.srcObject = meetLocalStream;
                  }
                }}
                autoPlay
                playsInline
            ...
            <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              ...
              {!isRegister && (
                <div style={{ display: 'flex', gap: '6px', padding: '4px', background: 'rgba(255,255,255,0.10)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.16)' }}>
                  <button type="button" onClick={() => setIsSSOLogin(false)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'var(--font-body)', background: !isSSOLogin ? 'rgba(255,255,255,0.90)' : 'transparent', color: !isSSOLogin ? '#000' : 'rgba(255,255,255,0.70)', transition: 'all 0.2s ease', boxShadow: !isSSOLogin ? 'inset 0 1px 0 rgba(255,255,255,1), 0 2px 6px rgba(0,0,0,0.15)' : 'none' }}>Standard Login</button>
                  <button type="button" onClick={() => setIsSSOLogin(true)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'var(--font-body)', background: isSSOLogin ? 'rgba(255,255,255,0.90)' : 'transparent', color: isSSOLogin ? '#000' : 'rgba(255,255,255,0.70)', transition: 'all 0.2s ease', boxShadow: isSSOLogin ? 'inset 0 1px 0 rgba(255,255,255,1), 0 2px 6px rgba(0,0,0,0.15)' : 'none' }}>SSO Sign In</button>
                </div>
              )}
              <div>
                <label style={{ display: 'block', color: 'rgba(255,255,255,0.65)', fontSize: '12px', fontWeight: 500, marginBottom: '6px' }}>Corporate Email</label>
                <input type="email" required className="input-field" value={email} onChange={e => setEmail(e.target.value)} placeholder="recruiter@company.com" />
              </div>
              {!isSSOLogin && (
                <div>
              ...
              {isRegister && (
                <>
                  <div style={{ display: 'flex', gap: '6px', padding: '4px', background: 'rgba(255,255,255,0.10)', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.16)' }}>
                    <button type="button" onClick={() => setIsJoinOrg(false)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'var(--font-body)', background: !isJoinOrg ? 'rgba(255,255,255,0.90)' : 'transparent', color: !isJoinOrg ? '#000' : 'rgba(255,255,255,0.70)', transition: 'all 0.2s ease' }}>New Organization</button>
                    <button type="button" onClick={() => setIsJoinOrg(true)} style={{ flex: 1, padding: '8px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: 600, fontFamily: 'var(--font-body)', background: isJoinOrg ? 'rgba(255,255,255,0.90)' : 'transparent', color: isJoinOrg ? '#000' : 'rgba(255,255,255,0.70)', transition: 'all 0.2s ease' }}>Join Team Code</button>
                  </div>
                  {!isJoinOrg ? (
                    <div>
                      <label style={{ display: 'block', color: 'rgba(255,255,255,0.65)', fontSize: '12px', fontWeight: 500, marginBottom: '6px' }}>Organization Name</label>
                    ...
                    <div>
                      <label style={{ display: 'block', color: 'rgba(255,255,255,0.65)', fontSize: '12px', fontWeight: 500, marginBottom: '6px' }}>Workspace Invite Code</label>
                      <input type="text" required className="input-field" value={inviteCode} onChange={e => setInviteCode(e.target.value)} placeholder="E.g., C9A4F3D0" style={{ fontFamily: 'var(--font-mono)' }} />
                    </div>
                  )}
                </>
              )}
              ...
              <div style={{ display: 'flex', alignItems: 'center', margin: '4px 0', color: 'rgba(255,255,255,0.30)', fontSize: '11px' }}>
                <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.15)' }} />
                <span style={{ padding: '0 12px', textTransform: 'uppercase', letterSpacing: '1px' }}>or</span>
                <div style={{ flex: 1, height: '1px', background: 'rgba(255,255,255,0.15)' }} />
              </div>
              <button type="button" onClick={() => setShowGoogleModal(true)} className="btn-secondary" style={{ width: '100%', justifyContent: 'center', gap: '10px', fontSize: '14px', padding: '12px', borderRadius: '14px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '16px' }}>
        ...
        <div className="glass-panel animate-fade-in" style={{ width: '100%', maxWidth: '480px', padding: '36px', textAlign: 'center' }}>
          <div style={{ marginBottom: '28px' }}>
            <div style={{ display: 'inline-flex', padding: '12px', background: 'var(--accent-purple-glow)', borderRadius: '12px', marginBottom: '16px', color: 'var(--accent-gold)' }}>
                    ...
                    <div className="glass-panel" style={{ padding:'20px', marginTop:'8px' }}>
                      ...
                      <div style={{ display:'flex', gap:'10px' }}>
                        ...
                        <button onClick={handleRoadmap} disabled={academyRoadmapLoading}
                          ...
                          {academyRoadmapLoading ? '⏳ Building…' : '🗺️ Generate'}
                        </button>
                      </div>
                      {academyRoadmap && (
